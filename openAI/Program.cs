...
internal class Program
{
	...
	private static async Task Main(string[] args)
	{
        ...
        async Task OnMessage(Message msg, UpdateType type)
		{
			...
			switch (msg.Text)
			{
				...
				case "/clear":
					...
					else
					{
						await bot.SendTextMessageAsync(
							chatId: msg.Chat.Id,
							text: TextsForMessages("/clear_else")
                        );
					}
					return;
			}
			...
			try
			{
				...
				var initialMessage = await bot.SendTextMessageAsync(
					chatId: msg.Chat.Id,
					text: TextsForMessages("WaitAnswer"),
			...
			catch (Exception ex)
			{
				...
				await bot.SendTextMessageAsync(
					chatId: msg.Chat.Id,
					text: TextsForMessages("ErrorMessage")
				);
			}
		}
		...
		static string ConvertToHtml(string message) // форматирование ответа от API
		{
